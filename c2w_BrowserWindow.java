...
public class c2w_BrowserWindow extends Stage {
    ...
    c2w_webEngine.getLoadWorker().stateProperty().addListener((o,oldVal,newVal)->{
        ...
        switch(newVal){
            ...
            break;
            case SUCCEEDED:
            status.setText("Status: web page has been successfully fetched");
            break;
            case FAILED:
             status.setText("Status : Loading of the web page has failed");
             break;
             case CANCELLED:
             status.setText("Status:Loading of th web page has been cancelled");
             break;
        }
    ...
    c2w_webEngine.setPromptHandler(promptdata->
        SimpleDialogs.prompt(prompt.Message(),"Query from web page",promptdata.getDefaultValue())
    );
...
c2w_webEngine.load(url);
    ...
    private MenuBar makeMenuBar(){
        ...
        newWin.setOnAction(e -> c2w_owner.c2w_newBrowserWindow(c2w_url:null));
        ...
        open.setOnAction(e ->{
            ...
            String url = SimpleDialogs.prompt(
                "Enter the url of the page that you want",title,"Get url");
    ...
    private void populateWindowMenu(){
        ...
        if(windows.size()>1){
            ...
            for(c2w_BrowserWindow:windows){
                ...
                item.setAction(e->win.requestFocus());
                if(window==this){
